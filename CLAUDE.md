# CLAUDE.md — ImplicitEx repository instructions for Claude Code
...
**May read:** All files in the repository, including Codex's operating
...
Available Claude subagents are defined in `.claude/agents/`. Each operates under
the same governance constraints as the orchestrating session.

Current agents:
- **scope-sentinel** — zero write authority; verifies proposed and completed
  changes against the active lane before and after implementation. Invoke before
  beginning implementation and again before committing.

## Cross-platform coordination

Codex operates under the same doctrine. Read `AGENTS.md` to understand Codex's
operating constraints and what Codex subagents exist. Avoid duplicating work.
Pursue complementary roles based on the declared lane.
