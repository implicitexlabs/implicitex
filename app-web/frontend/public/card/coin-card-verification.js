...
(function () {
  ...
  var trustedKeyResolution = window.IX_COIN_CARD_TRUSTED_KEY_RESOLUTION || null;
  var TRUSTED_KEY_OUTCOMES = trustedKeyResolution && trustedKeyResolution.TRUSTED_KEY_OUTCOMES || Object.freeze({
  ...
  function isTrustedKeySourceAvailable() {
    if (trustedKeyResolution && typeof trustedKeyResolution.isTrustedKeySourceAvailable === 'function') {
      return trustedKeyResolution.isTrustedKeySourceAvailable();
    }
    return false;
  }

  function getTrustedKeyRecord(keyId) {
    if (trustedKeyResolution && typeof trustedKeyResolution.getTrustedKeyRecord === 'function') {
      return trustedKeyResolution.getTrustedKeyRecord(keyId);
    }
    return null;
  }

  function resolveTrustedKeyRecord(keyId, context) {
    if (trustedKeyResolution && typeof trustedKeyResolution.resolveTrustedKeyRecord === 'function') {
      return trustedKeyResolution.resolveTrustedKeyRecord(keyId, context);
    }
    return {
      outcome: TRUSTED_KEY_OUTCOMES.TRUSTED_KEY_SOURCE_UNAVAILABLE,
      keyId: keyId || null,
      record: null,
      publicKey: null,
      reason: 'trusted-key-resolution-module-unavailable',
    };
  }
