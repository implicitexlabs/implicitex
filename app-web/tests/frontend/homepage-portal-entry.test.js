...
test('homepage replaces the embedded transfer shell with a portal-entry surface', () => {
  ...
  assert.doesNotMatch(html, /ethers\.umd\.min\.js/);
  assert.doesNotMatch(html, /walletconnect-provider\.js/);
  assert.doesNotMatch(html, /transfer-status\.js/);
  assert.doesNotMatch(html, /receipt-schema\.js/);
  assert.doesNotMatch(html, /receipt-integrity\.js/);
  assert.doesNotMatch(html, /proof-packet\.js/);
  assert.doesNotMatch(html, /receipt-store\.js/);
  assert.doesNotMatch(html, /ix-execution\.js/);
  assert.doesNotMatch(html, /wallet\.js/);
  assert.doesNotMatch(html, /portal-display\.js/);
  assert.doesNotMatch(html, /coincard\.js/);
  assert.doesNotMatch(html, /feeds\.js/);
  assert.doesNotMatch(html, /telemetry\.js/);
  assert.doesNotMatch(html, /rehydrate\.js/);
  assert.doesNotMatch(html, /coincard-handoff\.js/);
  assert.doesNotMatch(html, /components\/companion\/companion\.js/);
  assert.doesNotMatch(html, /config\/chains\.js/);
  ...
  assert.doesNotMatch(mobileMenuJs, /window\.IX/);
  assert.doesNotMatch(mobileMenuJs, /IX_CHAINS/);
  assert.doesNotMatch(mobileMenuJs, /openOrConnect|requestAccountSelection|disconnect\(\{ revokeProvider: true \}\)/);
});
